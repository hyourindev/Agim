...
VMResult vm_run(VM *vm) {
    ...
    for (;;) {
        ...
        case OP_HTTP_GET: {
            ...
            /* Use safe HTTP client instead of system() */
            HttpResponse *resp = http_get(url->as.string->data);
            if (!resp) {
                vm_push(vm, value_result_err(value_string("HTTP request failed")));
                break;
            }
            if (resp->error) {
                /* Request failed - use error message if available */
                vm_push(vm, value_result_err(value_string(resp->error)));
                http_response_free(resp);
            } else {
                Value *result = value_string(resp->body ? resp->body : "");
                http_response_free(resp);
        ...
        case OP_HTTP_POST: {
            ...
            if (!url || !body || !value_is_string(url) || !value_is_string(body)) {
                vm_set_error(vm, "URL and body must be strings");
                return VM_ERROR_TYPE;
            }
            /* Use safe HTTP client instead of system() */
            HttpResponse *resp = http_post(url->as.string->data,
                                           body->as.string->data,
                                           "application/json");
            if (!resp) {
                vm_push(vm, value_result_err(value_string("HTTP request failed")));
        ...
        case OP_HTTP_PUT: {
            ...
            HttpResponse *resp = http_put(url->as.string->data,
                                          body->as.string->data,
                                          "application/json");
            if (!resp) {
                vm_push(vm, value_result_err(value_string("HTTP PUT failed")));
                break;
            }
            if (resp->error) {
                vm_push(vm, value_result_err(value_string(resp->error)));
        ...
        case OP_HTTP_DELETE: {
            ...
            HttpResponse *resp = http_delete(url->as.string->data);
            if (!resp) {
                vm_push(vm, value_result_err(value_string("HTTP DELETE failed")));
                break;
            }
            if (resp->error) {
                vm_push(vm, value_result_err(value_string(resp->error)));
        ...
        case OP_HTTP_PATCH: {
            ...
            HttpResponse *resp = http_patch(url->as.string->data,
                                            body->as.string->data,
                                            "application/json");
            if (!resp) {
                vm_push(vm, value_result_err(value_string("HTTP PATCH failed")));
                break;
            }
            if (resp->error) {
                vm_push(vm, value_result_err(value_string(resp->error)));
        ...
        case OP_HTTP_REQUEST: {
            ...
            if (!method || !value_is_string(method) || !url || !value_is_string(url)) {
                vm_set_error(vm, "method and URL must be strings");
                return VM_ERROR_TYPE;
            }

            /* Convert headers map to array of strings */
            const char **headers = NULL;
            size_t header_count = 0;
            char **header_storage = NULL;

            if (headers_val && value_is_map(headers_val)) {
                Map *map = headers_val->as.map;
                header_count = map->size;
                if (header_count > 0) {
                    headers = malloc((header_count + 1) * sizeof(char*));
                    header_storage = malloc(header_count * sizeof(char*));
                    if (headers && header_storage) {
                        size_t idx = 0;
                        /* Iterate through buckets */
                        for (size_t i = 0; i < map->capacity && idx < header_count; i++) {
                            MapEntry *entry = map->buckets[i];
                            while (entry && idx < header_count) {
                                ...
                                if (key_str && val_str) {
                                    ...
                                    if (header_storage[idx]) {
                                        ...
                                        headers[idx] = header_storage[idx];
                                        idx++;
                                    }
                                }
                                entry = entry->next;
            ...
            const char *body_str = (body && value_is_string(body)) ? body->as.string->data : NULL;
            HttpResponse *resp = http_request_generic(method->as.string->data,
                                                       url->as.string->data,
                                                       body_str, headers);
        ...
        case OP_WS_CONNECT: {
            ...
            WSError error;
            WebSocket *ws = ws_connect(url->as.string->data, 30000, &error);
            if (!ws) {
                vm_push(vm, value_nil());
                break;
            }
            /* Return a handle with the WebSocket pointer */
            Value *handle = value_map();
            map_set(handle, "url", url);
            map_set(handle, "type", value_string("websocket"));
            map_set(handle, "_ws", value_int((int64_t)(uintptr_t)ws));
            vm_push(vm, handle);
            break;
        }
        ...
        case OP_WS_SEND: {
            ...
            if (!handle || !value_is_map(handle) || !msg || !value_is_string(msg)) {
                vm_set_error(vm, "ws_send requires handle and message string");
                return VM_ERROR_TYPE;
            }
            Value *ws_ptr = map_get(handle, "_ws");
            if (!ws_ptr || !value_is_int(ws_ptr)) {
                vm_push(vm, value_bool(false));
                break;
            }
            WebSocket *ws = (WebSocket *)(uintptr_t)ws_ptr->as.integer;
            if (!ws || !ws_is_connected(ws)) {
                vm_push(vm, value_bool(false));
                break;
            }
            /* Send the message */
            bool ok = ws_send_text(ws, msg->as.string->data);
        ...
        case OP_WS_RECV: {
            ...
            if (!handle || !value_is_map(handle)) {
                vm_set_error(vm, "ws_recv requires handle");
                return VM_ERROR_TYPE;
            }
            Value *ws_ptr = map_get(handle, "_ws");
            if (!ws_ptr || !value_is_int(ws_ptr)) {
            ...
            /* Get timeout (-1 = block forever by default) */
            int timeout_ms = -1;
            if (timeout_val && value_is_int(timeout_val)) {
                timeout_ms = (int)timeout_val->as.integer;
            }
            /* Receive message */
            size_t len;
            int opcode;
            char *data = ws_recv(ws, &len, &opcode, timeout_ms);
            if (data) {
                vm_push(vm, value_string(data));
                free(data);
            } else {
                vm_push(vm, value_nil());
            }
        ...
        case OP_HTTP_STREAM: {
            ...
            /* Create stream using safe HTTP client */
            HttpStream *stream = http_stream_get(url->as.string->data);
            if (!stream) {
                vm_push(vm, value_nil());
                break;
            }
            /* Return a handle with the stream pointer */
            Value *handle = value_map();
            map_set(handle, "url", url);
            map_set(handle, "type", value_string("sse"));
            /* Store stream pointer as integer (cast is safe, restored on read) */
            map_set(handle, "_stream", value_int((int64_t)(uintptr_t)stream));
            vm_push(vm, handle);
            break;
        }
        ...
        case OP_STREAM_READ: {
            ...
            if (!stream_ptr || !value_is_int(stream_ptr)) {
                vm_push(vm, value_nil());
                break;
            }
            HttpStream *stream = (HttpStream *)(uintptr_t)stream_ptr->as.integer;
            if (!stream) {
                vm_push(vm, value_nil());
                break;
            }
            ...
            size_t chunk_len;
            char *chunk = http_stream_read(stream, &chunk_len);
            if (chunk) {
                vm_push(vm, value_string(chunk));
                free(chunk);
            } else if (http_stream_done(stream)) {
                vm_push(vm, value_nil());
            } else {
            ...
            if (handle && value_is_map(handle)) {
                ...
                if (stream_ptr && value_is_int(stream_ptr)) {
                    ...
                    if (stream) {
                        ...
                        map_set(handle, "_stream", value_nil());
                    }
                }
            }
            vm_push(vm, value_nil());
