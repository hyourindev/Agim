/*
 * Agim - Built-in Tools
 *
 * Tool registration and execution framework.
 *
 * Copyright (c) 2025 Agim Language Contributors
 * SPDX-License-Identifier: MIT
 */

#ifndef AGIM_BUILTIN_TOOLS_H
#define AGIM_BUILTIN_TOOLS_H

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

/* Forward declarations */
typedef struct Block Block;
typedef struct Value Value;

/*============================================================================
 * Tool Function Type
 *============================================================================*/

/**
 * Built-in tool function type.
 *
 * @param block     The calling block
 * @param args      Array of argument values
 * @param arg_count Number of arguments
 * @param context   User-provided context
 * @return          The result value, or NULL on error
 */
typedef Value *(*ToolFunction)(Block *block, Value **args, size_t arg_count,
                               void *context);

/*============================================================================
 * Tool Parameter Schema
 *============================================================================*/

/**
 * Parameter type for tool schema.
 */
typedef enum ToolParamType {
    TOOL_PARAM_STRING,
    TOOL_PARAM_INT,
    TOOL_PARAM_FLOAT,
    TOOL_PARAM_BOOL,
    TOOL_PARAM_ARRAY,
    TOOL_PARAM_MAP,
    TOOL_PARAM_ANY,
} ToolParamType;

/**
 * Parameter definition for tool schema.
 */
typedef struct ToolParam {
    const char *name;
    const char *description;
    ToolParamType type;
    bool required;
    Value *default_value;       /* NULL if no default */
} ToolParam;

/*============================================================================
 * Tool Registration Entry
 *============================================================================*/

typedef struct Tool {
    const char *name;
    const char *description;    /* Human-readable description */
    ToolFunction func;
    size_t min_args;
    size_t max_args;
    uint32_t required_caps;     /* Required capabilities */
    void *context;
    ToolParam *params;          /* Parameter schema (NULL if none) */
    size_t param_count;
    struct Tool *next;
} Tool;

/*============================================================================
 * Tool Registry
 *============================================================================*/

typedef struct ToolRegistry {
    Tool *tools;
    size_t count;
} ToolRegistry;

/*============================================================================
 * Tool API
 *============================================================================*/

/**
 * Initialize tool registry.
 */
void tools_init(ToolRegistry *registry);

/**
 * Free tool registry.
 */
void tools_free(ToolRegistry *registry);

/**
 * Register a tool (simple version without schema).
 */
bool tools_register(ToolRegistry *registry, const char *name,
                    ToolFunction func, size_t min_args, size_t max_args,
                    uint32_t required_caps, void *context);

/**
 * Register a tool with full schema.
 */
bool tools_register_with_schema(ToolRegistry *registry, const char *name,
                                const char *description,
                                ToolFunction func, size_t min_args, size_t max_args,
                                uint32_t required_caps, void *context,
                                ToolParam *params, size_t param_count);

/**
 * Unregister a tool.
 */
void tools_unregister(ToolRegistry *registry, const char *name);

/**
 * Call a tool by name.
 */
Value *tools_call(ToolRegistry *registry, Block *block,
                  const char *name, Value **args, size_t arg_count);

/**
 * Get list of registered tools.
 */
const Tool *tools_list(ToolRegistry *registry);

/**
 * Get tool count.
 */
size_t tools_count(ToolRegistry *registry);

/**
 * Find a tool by name.
 * Returns NULL if not found.
 */
Tool *tools_find(ToolRegistry *registry, const char *name);

/**
 * Get OpenAI-compatible JSON schema for a tool.
 * Returns an allocated string (must be freed by caller).
 */
char *tools_get_schema_json(const Tool *tool);

/**
 * Get JSON schema for all tools (OpenAI function calling format).
 * Returns an allocated string (must be freed by caller).
 */
char *tools_get_all_schemas_json(ToolRegistry *registry);

/**
 * Get tool list as a Value (array of maps with name, description, params).
 */
Value *tools_list_as_value(ToolRegistry *registry);

/*============================================================================
 * Built-in Tools
 *============================================================================*/

/**
 * Register default built-in tools (print, type, len, etc.)
 */
void tools_register_builtins(ToolRegistry *registry);

/*============================================================================
 * Bytecode Tool Registration
 *============================================================================*/

/* Forward declarations */
typedef struct VM VM;
typedef struct Bytecode Bytecode;

/**
 * Context for calling bytecode-defined tools.
 */
typedef struct BytecodeToolContext {
    VM *vm;
    size_t func_index;
    Bytecode *code;
} BytecodeToolContext;

/**
 * Register all tools defined in bytecode into the registry.
 * Each tool will invoke the corresponding bytecode function when called.
 *
 * @param registry  The tool registry to register into
 * @param code      The bytecode containing tool definitions
 * @param vm        The VM to use for executing tool functions
 */
void tools_register_from_bytecode(ToolRegistry *registry, Bytecode *code, VM *vm);

#endif /* AGIM_BUILTIN_TOOLS_H */
